...
insert into provider (pk,version,code, name, codespstace_pk,created,created_by,changed,changed_by) values  (nextval('provider_seq'),1, 'rut', 'Ruter',(select pk from codespace where xmlns='RUT'),now(),'test',now(),'test');