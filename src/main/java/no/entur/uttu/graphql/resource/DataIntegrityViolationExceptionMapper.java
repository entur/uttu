...
public class DataIntegrityViolationExceptionMapper
  ...
  public Response toResponse(DataIntegrityViolationException e) {
    ...
    if (e.getCause() instanceof ConstraintViolationException) {
      ...
      CodedError codedError = ConstraintViolationCodedError.fromConstraintName(
        constraintViolationException.getConstraintName()
      );
      ...
      return Response.status(Response.Status.OK).entity(errorResponseEntity).build();
    }
  ...
  private void init() {
    errorMessagePerConstraintMap.put(
      Constraints.NETWORK_UNIQUE_NAME,
      "A Network with this name already exists"
    );
    errorMessagePerConstraintMap.put(
      Constraints.FLEXIBLE_STOP_PLACE_UNIQUE_NAME,
      "A FlexibleStopPlace with this name already exists"
    );
    errorMessagePerConstraintMap.put(
    ...
    errorMessagePerConstraintMap.put(
      ...
      "A JourneyPattern with this name already exists"
    );
    errorMessagePerConstraintMap.put(
      Constraints.SERVICE_JOURNEY_UNIQUE_NAME,
      "A ServiceJourney with this name already exists"
    );

    errorMessagePerConstraintMap.put(
      Constraints.CODESPACE_UNIQUE_XMLNS,
      "A Codespace with this xmlns already exists"
    );
    errorMessagePerConstraintMap.put(
