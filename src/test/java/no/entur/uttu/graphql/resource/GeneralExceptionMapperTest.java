...
public class GeneralExceptionMapperTest {
  ...
  public void rawAccessDeniedExceptionYieldsForbidden() {
    Response rsp = new GeneralExceptionMapper()
      .toResponse(new AccessDeniedException("Nope"));
  ...
  public void nestedAccessDeniedExceptionYieldsForbidden() {
    Response rsp = new GeneralExceptionMapper()
      .toResponse(new TransactionSystemException("", new AccessDeniedException("Nope")));
    Assert.assertEquals(Response.Status.FORBIDDEN.getStatusCode(), rsp.getStatus());
    Assert.assertEquals(
      "Nope",
      ((ErrorResponseEntity) rsp.getEntity()).errors.get(0).message
    );
  }
  ...
  public void nestedNotAuthorizedExceptionYieldsUnauthorized() {
    Response rsp = new GeneralExceptionMapper()
      .toResponse(new TransactionSystemException("", new NotAuthorizedException("Njet")));
